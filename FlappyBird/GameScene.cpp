...
GameScene::GameScene()
{
	...
	// 添加 ready 图片
	ready = gcnew Sprite(ResLoader::getKeyFrame("text_ready"));
	ready->setPos(Window::getWidth() / 2, Window::getHeight() / 2 - 70);
	this->addChild(ready);

	// 添加教程图片
	tutorial = gcnew Sprite(ResLoader::getKeyFrame("tutorial"));
...
void GameScene::hitHappend()
{
	...
	// 闪动白屏
	auto white = gcnew Sprite(ResLoader::getKeyFrame("white"));
	white->setAnchor(0, 0);
	white->setOpacity(0);
